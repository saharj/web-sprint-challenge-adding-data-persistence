...
1. Explain the difference between `Relational Databases` and `SQL`.
   Data in Relational Databases are stored in tabular format whereas in sql, data is stored in tables. The relation between these dbs are stored in a table.

2. Why do tables need a `primary key`?
   Tables need a unique key for each data, so that it can be referenced from out of the table, and that it would be possible to prevent duplicates.

3. What is the name given to a table column that references the primary key on another table.
   id

4. What do we need in order to have a _many to many_ relationship between two tables.
   We would need foreign keys which are references to the primary key of each table.
